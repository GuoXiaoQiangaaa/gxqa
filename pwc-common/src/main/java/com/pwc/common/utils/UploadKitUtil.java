...
public class UploadKitUtil {
     ...
     * @return
     */
    public static UploadFileEntity uploadFile(MultipartFile multipartFile, String filePath, boolean useNewName, boolean isAbsolutePath) {

        try {
            UploadFileEntity f = transferFile(filePath,multipartFile,useNewName, isAbsolutePath);
            if(f!=null){
                return f;
            }
        } catch (Exception e) {
     ...
     * @throws Exception
     */
    private static UploadFileEntity transferFile(String filePath,MultipartFile file, boolean useNewName, boolean isAbsolutePath) throws Exception{
        // 存储路径
        String fileAbsolutePath = filePath;
        if (!isAbsolutePath) {
            fileAbsolutePath = ClassUtil.getClassPath() + filePath;
        }
        /** 取得上传文件 */
...
//        Thumbnails.of(file.getInputStream()).scale(0.25f).outputQuality(0.2f).toFile(filePath + "/" + abbreviationName);
        ...
        upload.setFileSize(fileSize.toString());
